...
{
    /// <summary>
    /// Evaluates a binary vector expression on two per-particle vector inputs and writes the result to
    /// a vector attribute of each new particle. The colour and alpha attributes hold a fraction, so a
    /// result written to one of them is clamped to the unit range before the optional normalization.
    /// </summary>
    /// <seealso href="https://s2v.app/SchemaExplorer/cs2/particles/C_INIT_SetVectorAttributeToVectorExpression">C_INIT_SetVectorAttributeToVectorExpression</seealso>
